...
open class LLPhotoBrowserViewController: LLBrowserViewController {
    ...
    func loadBigImageWithItem(item: LLBrowserModel, cell: LLBrowserCollectionViewCell, rect: CGRect) {
        ...
        imageView?.kf.setImage(with: URL(string: item.imageURL!)!, placeholder: (item.sourceImageView != nil && item.sourceImageView?.image != nil) ? item.sourceImageView?.image : UIImage.init(named: "LLPhotoBrowser.bundle/ll_placeholder"), options: nil, progressBlock: nil, completionHandler: { (image, error, cacheType, imageURL) in
